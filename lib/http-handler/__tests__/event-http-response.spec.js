...
const { EventHttpResponse, response } = require('../../http-handler');
...
describe('EventHttpResponse', () => {
  ...
  test('deve criar uma resposta inicializando pelo o construtor', () => {
    ...
    expect(resp.statusDescription).toEqual('201 Created');
  ...
  test('deve criar uma resposta com uma imagem', () => {
    ...
    expect(resp.statusDescription).toEqual('200 OK');
  ...
  test('deve formatar a resposta para a integração da Lambda', () => {
    ...
    expect(lambdaResponse.statusDescription).toEqual('201 Created');
    ...
    try {
      Object.keys(http.STATUS_CODES)
        ...
        .forEach((code) => {
          ...
          expect(resp.statusDescription).toEqual(`${resp.statusCode} ${http.STATUS_CODES[resp.statusCode]}`);
        });
    } catch (error) {
